...
pm.service('sharedDataService', function ($window, $dialogs, $timeout) {
  ...
  this.extratime_wait = [
    {
      ...
      extratime_estimation_min: 0,
      extratime_estimation_max: 0
    },
    {
      occupancy: 10,
      difficulty_level: 'Null',
    ...
    },
    {
      ...
      extratime_estimation_min: 1,
      extratime_estimation_max: 1
    },
    {
      occupancy: 70,
      difficulty_level: 'Medium',
      extratime_estimation_min: 1,
      extratime_estimation_max: 3
    },
    {
      occupancy: 80,
      difficulty_level: 'High',
      extratime_estimation_min: 2,
      extratime_estimation_max: 5
    },
    {
      occupancy: 90,
      difficulty_level: 'Very High',
      extratime_estimation_min: 3,
      extratime_estimation_max: 10
    },
    {
      occupancy: 100,
      difficulty_level: 'Impossible',
      extratime_estimation_min: 5,
      extratime_estimation_max: 15
    },
    ];
  ...
  this.getSharedPaymentMethods = function () {
  ...
  this.getTotalSlotsInZone = function (z_id, occStreetList, vehicleType) {
    ...
    if (occStreetList != null && occStreetList.length > 0) {
      for (var i = 0; i < occStreetList.length; i++) {
        var found = false;
        for (var j = 0;
          (j < occStreetList[i].zones.length) && !found; j++) {
    ...
    if (profitPmList != null && profitPmList.length > 0) { // map page case
      for (var i = 0; i < profitPmList.length; i++) {
        ...
        if (profitPmList[i].zones) {
          for (var j = 0;
            (j < profitPmList[i].zones.length && !found); j++) {
  ...
  this.loadStreetsFromZone = function (z_id, streetList) {
    ...
    if (streetList != null && streetList.length > 0) {
      for (var i = 0; i < streetList.length; i++) {
        var found = false;
        for (var j = 0;
          (j < streetList[i].zones.length) && !found; j++) {
  ...
  this.getStreetsInZoneProfit = function (z_id, profitStreetList, profitStreetDataList, profitPMs) {
    ...
    if (profitStreetList != null && profitStreetList.length > 0) { // map page case
      for (var i = 0; i < profitStreetList.length; i++) {
        ...
        // all zones case
        for (var j = 0;
          (j < profitStreetList[i].zones.length && !found); j++) {
      ...
      for (var i = 0; i < profitStreetDataList.length; i++) {
        ...
        if (profitStreetDataList[i].zones != null) {
          for (var j = 0;
            (j < profitStreetDataList[i].zones.length && !found); j++) {
...
pm.factory('getMyMessages', function ($http, $q) {
  ...
  return {
    promiseToHaveData: promiseToHaveData
  };
