...
pm.controller('ViewCtrlGmap', ['$scope', '$http', '$route', '$routeParams', '$rootScope', 'localize', 'sharedDataService', 'initializeService', 'gMapService', 'areaService', 'streetService', 'parkingMeterService', 'structureService', 'bikePointService', 'zoneService', '$timeout',
    ...
    $scope.addZonePoligonToList = function (zone, index) {
      switch (index) {
      case 0:
        $scope.mapZones0.push(zone);
        break;
      case 1:
        $scope.mapZones1.push(zone);
        break;
      case 2:
        $scope.mapZones2.push(zone);
        break;
      case 3:
        $scope.mapZones3.push(zone);
        break;
      case 4:
        $scope.mapZones4.push(zone);
        break;
      default:
        break;
      }
      ...
      if (zones && zones.length > 0) {
        for (var i = 0; i < streets.length; i++) {
          if (streets[i].zones) {
            var found = false;
            for (var j = 0;
              (j < streets[i].zones.length) && !found; j++) {
      ...
      if (zones && zones.length > 0) {
        for (var i = 0; i < areas.length; i++) {
          if (areas[i].zones) {
            var found = false;
            for (var j = 0;
              (j < areas[i].zones.length) && !found; j++) {
              for (var z = 0; z < zones.length; z++) {
