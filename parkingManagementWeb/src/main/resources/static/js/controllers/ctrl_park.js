...
pm.controller('ParkCtrl', ['$scope', '$http', '$routeParams', '$rootScope', '$route', '$location', '$dialogs', 'sharedDataService', '$filter', 'initializeService', 'gMapService', 'areaService', 'streetService', 'zoneService', 'structureService', 'parkingMeterService', 'bikePointService', 'getMyMessages', '$timeout',
    ...
    $scope.multiSelSettings = {
      displayProp: 'name' - 'submacro'
    };
    ...
    $scope.initComponents = function () {
      if ($scope.editparktabs == null || $scope.editparktabs.length == 0) {
        ...
        var zone_tab_list = [];
        var area_tab_obj = {
          title: 'manage_area_tab',
          index: 1,
          content: "partials/edit/tabs/edit_area.html"
        };
        var street_tab_obj = {
          title: 'manage_street_tab',
          index: 7,
          content: "partials/edit/tabs/edit_street.html"
        };
        var ps_tab_obj = {
          title: 'manage_structure_tab',
          index: 8,
          content: "partials/edit/tabs/edit_parkingstructure.html"
        };
        var pm_tab_obj = {
          title: 'manage_parkingmeter_tab',
          index: 9,
          content: "partials/edit/tabs/edit_parkingmeter.html"
        };
        var bp_tab_obj = {
          title: 'manage_bikepoint_tab',
          index: 10,
          content: "partials/edit/tabs/edit_bike.html"
        };
    ...
    $scope.checkIfAlreadyExist = function (pm_code) {
      ...
      if (!$scope.isEditing) {
        for (var i = 0;
          ((i < $scope.pmeterWS.length) && !found); i++) {
    ...
    $scope.addOpeningPeriod = function (start, end) {
      ...
      if (start.getTime() > end.getTime()) {
        ...
        if ($scope.openingPeriods == null || $scope.openingPeriods.length == 0) {
          ...
          var timeStor = null;
          for (var i = 0;
            (i < $scope.openingPeriods.length && !inserted); i++) {
    ...
    $scope.addZonePoligonToList = function (zone, index) {
      switch (index) {
      case 0:
        $scope.zone_polygons0.push(zone);
        break;
      case 1:
        $scope.zone_polygons1.push(zone);
        break;
      case 2:
        $scope.zone_polygons2.push(zone);
        break;
      case 3:
        $scope.zone_polygons3.push(zone);
        break;
      case 4:
        $scope.zone_polygons4.push(zone);
        break;
      default:
        break;
      }
    ...
    $scope.inithializeVehicleTypeList = function (activedSlotsConfiguration, type) {
      ...
      if (activedSlotsConfiguration) {
        for (var i = 0; i < activedSlotsConfiguration.length; i++) {
          var found = false;
          for (var j = 0;
            (j < $scope.newVehicleList.length) && !found; j++)
    ...
    $scope.deleteVehicleTypeSlots = function (vehicleTypeSlots, type) {
      if (vehicleTypeSlots && vehicleTypeSlots.vehicleType) {
        ...
        if (type == 0) {
          for (var i = 0;
            (i < $scope.eStreet.slotsConfiguration.length) && !found; i++) {
          ...
          for (var i = 0;
            (i < $scope.parkingStructure.slotsConfiguration.length) && !found; i++) {
    ...
    function updatePaymentMethods(parkingMeters, paymentValues) {
      var newpm = [];
      for (var key in paymentValues.value) {
        if (paymentValues.value[key])
          newpm.push(key);
      }
      parkingMeters.paymentMethods = newpm;
      return parkingMeters
    }
    // Update parkingMeter Object
    $scope.updatePmeter = function (type, pm, form, area, geometry, zone0, zone1, zone2, zone3, zone4) {
      pm = updatePaymentMethods(pm, $scope.selection);
      if (!form.$valid) {
        $scope.isInit = false;
      } else {
